...
Only an explicit `terminated` preflight/launch/session failure that remains unresolved can become `tool-unavailable`. Then create the manifest, bind an independent terminal-evidence artifact, mark unrealized functional evidence `unverified`, record the exact non-secret terminal result, missing evidence, and one product impact, present those facts to the Owner, and stop. Only the Owner's later exact reply accepting continuation without the formal artifact can authorize the unavailable continuation path; generic permission and Agent-authored words cannot.
