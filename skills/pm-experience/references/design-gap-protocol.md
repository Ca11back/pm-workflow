...
Reuse the prior `.pen` only as input and always write fresh `.pen`/PNG targets. Preserve unaffected Journey/Coverage nodes. A new product decision or behavior gap stops local patching and routes to Definition; it does not trigger another automatic semantic-review round. A full review is followed by at most one focused recheck after Findings are addressed, and a changed approved baseline or artifact is required before a new full review.
