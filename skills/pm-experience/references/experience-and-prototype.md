...
First create `draft/experience/brief.md` with target, smallest scope, roles, pages/states, source behavior locators, fidelity, exclusions, authority, and route evidence; keep approval words/date `pending`. Show it in plain language and stop. When the Owner later replies explicitly, copy the real wording/date into the Brief and immediately call `approve-brief` with that final Brief plus only immutable route-specific reference/justification artifacts. The event binds the Brief; do not include the later Experience manifest, worksheet, Pen source, preview, or read-back. No Pen mutation precedes this event, and the approved Brief must not be edited afterward.

After `approve-brief`, generate the route evidence. For Pen, follow [pen-direct.md](pen-direct.md); for `existing-reference` and `not-needed`, record the selected immutable reference or justification. Copy and complete `experience-manifest.md` only after Brief approval. Show the complete preview or exact evidence and stop. `approve-preview` requires a separate explicit Owner reply and is the first event that binds the mutable manifest together with current source/preview/read-back or reference/justification hashes.

Only after both approvals may `freeze-candidate` create `CAND-*`. Missing save/read-back/export, route mismatch, stale hash, unresolved feedback, or secrets fail before snapshot.
