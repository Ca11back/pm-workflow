...
Copy [experience-brief.md](assets/experience-brief.md) to `draft/experience/brief.md`. Complete the semantic Brief, approval-bound Coverage IDs/runtime relationships, complete Journey IDs for entry/result/re-entry/recovery, and route decision while leaving the approval words/date as `pending`. A visible navigation, action, or return affordance must resolve to an included Coverage ID; omit it when its destination is outside the approved scope. Show the concrete scope and fidelity in plain language and stop. Keep the later manifest, Pen source, preview, read-back, and lifecycle fields out of this approval step. Do not replace the Brief template with an ad hoc file that omits either coverage map.
...
After `ready`, create `draft/experience/manifest.md` from [experience-manifest.md](assets/experience-manifest.md), copy every approved Coverage ID, relationship statement, and Journey ID unchanged, and complete the Brief-derived design, reachability/affordance read-back, save, preview, and visual-review sequence in the same process. Before presenting any non-visual correction, stop inventing affected behavior and finish one read-only sweep of all remaining approved Journeys so independent gaps can be consolidated. Only a proven `terminated` failure may create unavailable evidence. While launch is `running`, do not inspect final files as failure evidence, fill failure fields, offer a downgrade, or start another Pen process.
