...
After `ready`, create `draft/experience/manifest.md` from [experience-manifest.md](assets/experience-manifest.md), copy every approved Coverage ID, relationship statement, and Journey ID unchanged, and complete the Brief-derived design, reachability/affordance read-back, save, preview, and visual-review sequence in the same process. Only a proven `terminated` failure may create unavailable evidence. While launch is `running`, do not inspect final files as failure evidence, fill failure fields, offer a downgrade, or start another Pen process.
