...
For a multi-state Pen artifact, reconstruct each frame's runtime relationship from the approved Brief, Markdown locators, visible Pen node/read-back, and manifest. Require the same approval-bound Coverage ID and relationship statement throughout. Independently derive critical journeys from the authoritative roles, objects, lifecycle, Rules, and Scenarios instead of trusting the Brief's declared inventory: when an object persists or changes asynchronously, verify a later re-entry/retrieval path or an explicit not-applicable reason. Trace every visible navigation, action, and return affordance to a covered destination; a complete Coverage-ID list cannot excuse an unreachable flow or dangling target. Evidence-canvas layout is not runtime behavior authority; raise a Finding when relationship or journey evidence is missing, internally contradictory, or conflicts with approved behavior.
