...
Independently derive the functional representation obligations from authoritative roles, objects, lifecycle, Rules, Scenarios and Journeys before trusting the Brief or manifest inventories. Treat each Journey as an independently enterable goal or durable-object re-entry/recovery path; split independent lookup, cancellation, recovery, or later retrieval responsibilities rather than reducing rows. Identify each stable task-context Screen and primary job; approved/risk-material States with enough task context, visible delta, available/unavailable actions, feedback/status and recovery/re-entry; required content groups, functional regions and semantic controls; and every Step's source, visible trigger, feedback, result, failure/recovery and re-entry. Then compare that reconstruction with the Brief's Screen/State/Step contract, exact artifact locators, descendant read-back and rendered evidence. Compact cards pass only when they expose those obligations; a title, legend, or status label alone is not a substitute. Text that says “enter”, “choose”, “collection”, “continue”, or “done” is not a substitute for the required input, selection, collection, control, feedback or result structure.

Require each Coverage ID and relationship statement to agree across approved Brief, artifact/read-back, and manifest. Verify exact Screen/State/Step identity closure without treating those IDs as behavior authority. Independently derive critical journeys rather than trusting declared inventory: when an object persists or changes asynchronously, verify later re-entry/retrieval or an explicit not-applicable reason. Trace every visible navigation, action and return affordance to a covered destination; a complete Coverage list cannot excuse an unreachable flow, whole-frame hotspot, root-bounds-only read-back or dangling target. Evidence-canvas layout is not runtime behavior authority. Report over-compressed State evidence with exact `path#ID` locators and affected obligations; never use a page or node threshold.
...
On focused re-review, name the prior report/FND, exact `bundle_root` and bundle-relative locators checked, evidence, and conclusion. A Reviewer may verify `closed` or `withdrawn` in the new report; `pm-handoff` later records it in the mutable register. Allow at most one focused recheck after the single full review; scope/shared-model/permission/lifecycle or Experience-target/source/preview changes require a new full review. Routine consistency does not reopen a full cycle, and a newly invented product decision routes back instead of triggering another automatic review. Never mutate the Delivery, `.pen` source, or original report.
