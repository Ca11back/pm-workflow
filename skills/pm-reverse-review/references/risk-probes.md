...
- Before trusting the manifest, can the Reviewer independently derive each stable Screen job, material State, required content/functional region/semantic control, and Journey Step trigger/feedback/result/recovery/re-entry from Definition/Journey authority?
- Do artifact locators and descendant read-back prove the required input, choice, collection, action, feedback and result structures, rather than only root bounds, icons, labels or prose claiming those functions?
- Where different primary jobs share identical functional regions, is there a real task reason and does each still contain its required content/control evidence? Treat shared shells, arbitrary names, monochrome output and justified identical state structures as allowed; never use frame/node counts, keyword matches, visual novelty or aesthetic scoring as a pass/fail proxy.
...
- Are Journeys independently enterable goals or durable-object re-entry/recovery paths rather than row-count compression, and does each material State expose task context, visible delta, available/unavailable actions, feedback/status and recovery/re-entry? A compact card with only a title, legend, or status label is not evidence.
...
- Did the Owner confirm functional expression and scope only, with brand/aesthetic approval explicitly excluded and no claim that this inspection was real-user usability testing?
